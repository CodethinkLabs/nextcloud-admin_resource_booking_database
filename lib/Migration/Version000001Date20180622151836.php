...
class Version000001Date20180622151836 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, \Closure $schemaClosure, array $options) {
		...
		foreach($types as $type) {
			if (!$schema->hasTable('arb_db_' . $type)) {
				$table = $schema->createTable('arb_db_' . $type);
				...
				$table->setPrimaryKey(['id'], 'arb_db_' . $type . '_id_idx');
				$table->addIndex(['resource_id'], 'arb_db_' . $type . '_backendresource_idx');
				$table->addIndex(['email'], 'arb_db_' . $type . '_email_idx');
				$table->addIndex(['displayname'], 'arb_db_' . $type . '_displayname_idx');
			}
		}
